...
def run_protein_energy_landscape_calc( energy_fxn, restore, config, test_name ): 
	...
	xml_script =  config.benchmark_path + "tests/xml/make_depth_vs_tilt_energy_landscape.xml"
	print(xml_script)
	...
	for case in test_cases:
		...
		if (interface=="0"):
			start_z = ["-1", "-1", "-1", "-1", "0", "1", "1", "1"]
			end_z = ["-3", "-2", "-1", "0", "0", "0", "1", "2"]

		for j in range(len(start_z)):
 
			# Default composition is DLPC
			arguments = " -overwrite -out:nooutput -in:file:s " +  pdbfile + " -mp:setup:spanfiles " + spanfile + " -parser:script_vars sfxn_weights=" + energy_fxn + " -parser:script_vars interface=" + interface + " -parser::script_vars start_z=" + start_z[j] + " -parser::script_vars end_z=" + end_z[j] + " -parser::script_vars flag_1=" + flag_1[0] + " -parser:protocol " + xml_script 
