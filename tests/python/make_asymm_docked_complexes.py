...
def post_process_prepack_pdb( energy_fxn, config, targets, test_name, transformed=False ): 
  ...
  for i in range(0, len(target_ids)): 
    ...
    s = Template( "python3 $rosetta/../../tools/protein_tools/scripts/clean_pdb.py $title $chains")
...
def run_prepack_calc( energy_fxn, config, targets, test_name ): 
  ...
  for i in range(0, len(target_ids)): 
    ...
    # Generate a string of arguments from the case-specific variables
    s = Template( "-database $database_file -in:file:s $native -mp:setup:spanfiles $spanfile -score:weights $sfxn -docking:partners $partners -packing:pack_missing_sidechains 0 -nstruct 1 -out:path:all $outdir -mp:lipids:composition DLPC -mp:lipids:has_pore false" )
    arguments = s.substitute( database_file=database_file, native=native, spanfile=spanfile, sfxn=energy_fxn, outdir=casedir, partners=partners )
