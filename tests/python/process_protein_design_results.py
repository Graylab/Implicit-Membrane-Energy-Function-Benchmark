...
#@author: Rituparna Samanta (rsamant2@jh.edu)
#RS have added sequence recovery of the whole new protein design. 
...
def calculate_sequence_matches(seq, base):
    import textdistance
    
    list_matches = []
    if(len(seq)!=len(base)):
            # sys.exit('seq lengths donot match')
            return([0])
    ld = textdistance.levenshtein(seq, base)
    list_matches.append(len(base) - ld)
    return list_matches

def calculate_sequence_recovery_alldesigns(pdb_name, path_to_folder, native_file):
	from Bio import SeqIO
	from os import listdir


	filenames = listdir(path_to_folder)
	pdb_files= [ path_to_folder+filename for filename in filenames if filename.endswith( '.pdb' ) ]
	if(not os.path.isfile(native_file)):
		print('getting out of the function')
		return()
	with open(native_file,'r') as file:
		native_seq=[record.seq for record in SeqIO.parse(file,'pdb-atom')]
	full_nativeseq=''
	for x in native_seq:
		full_nativeseq=full_nativeseq+x
	print(native_file,full_nativeseq)
	file_output = open(path_to_folder+'/fasta_alldes.txt', "a")

	design_name=[]
	sequence_recovery=[]
	file_output.write(">{} \n {} \n".format('native'+pdb_name,full_nativeseq) )
	for fileName in pdb_files:
		with open(fileName,'r') as file:
			seq=[record.seq for record in SeqIO.parse(file,'pdb-atom')]
		file.close()
		full_seq=''
		for x in seq:
			full_seq=full_seq+x
		list_matches = calculate_sequence_matches(full_seq, full_nativeseq)    
		# print(list_matches)
		# print(len(full_seq))
		# print(len(full_nativeseq))
		if(len(full_seq)!=len(full_nativeseq)):
			print('-================for {} lengths do not match============='.format(fileName.split('/')[-1]))
		recovery_per_seq = [float(t)/len(full_nativeseq) for t in list_matches]
		design_name.append(fileName.split('/')[-1])
		sequence_recovery.append(recovery_per_seq[0])
		file_output.write(">{}\n {} \n".format(fileName.split('/')[-1],full_seq) )
		# file_output.write("{} {} {} {}\n".format(design_name, len(full_nativeseq), len(full_seq), recovery_per_seq[0]))
    	# file_output.close()        
	property = np.stack((design_name,sequence_recovery ), axis=1)
	df_if = pd.DataFrame(property, columns=['name', 'seq_recov'])
	df_if.to_csv(path_to_folder+'/seq_recov_alldes.csv', sep=" ", index=False)
            
def folder_list(base_dir):
	import os
	#print all subdirectories
	subdirs=[base_dir+d+'/' for d in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir,d))]
	# print("=========printing subdirectories==========")
	# print(subdirs)
	# print(len(subdirs))
	# sys.exit()
	for dir in subdirs[50:169]:
		# print(dir)
		pdb_name=dir.split('/')[-2]
		print(pdb_name)
		for line in open(base_dir+'natives.list').readlines():
			if pdb_name in line:
				native_file = line 
		# print(native_file[:-1])
		calculate_sequence_recovery_alldesigns(pdb_name, dir, native_file[:-1])
...
def main( args ) : 
	...
	working_dir = Options.basedir 
	#+ "data/" + Options.energy_fxn + "/sequence-recovery"
	...
	#Analyze the sequence recovery of all designs for each pdb.
	folder_list(working_dir)
