...
def run_peptide_energy_landscape_calc( energy_fxn, config, test_name, targets, xml, pH_mode=False, pH=7 ): 
	...
	outdir = config.benchmark_path + "data/" + energy_fxn + "/" + test_name + "/franklin2021_fimm1_f19_loop20"
        #franklin2021_fimm0.02_fbilayer0.02_menvph__RALFscale_loop20" 
	print("outdir:~",outdir)
	...
	for case in test_cases:
		...
		if ( "tm-peptide-tilt-angle" in test_name ):
			interface = "0"
			start_z = ["-60","-50","-40","-30","-20","-10"]#,"-50","-40","-30","-20","-10","0","10","20","30","40"] #["-40", "-30", "-20", "-10", "0", "10", "20", "30"]
			end_z = ["-50","-40","-30","-20","-10","0"]#,"-40","-30","-20","-10","0","10","20","30","40","50"] #-30", "-20", "-10", "0", "10", "20", "30", "40"]
        
		if ( "adsorbed-peptide-tilt-angle" in test_name ):
			interface = "1"
			if( "tilt_angle/A4_design_surface_ahelices" not in targets ):
				flag_axis = ["2.0"]
			start_z = ["-60","-50","-40","-30","-20","-10","0","10","20","30","40","50"]#,"-50","-40","-30","-20","-10","0","10","20","30","40"] #["-40", "-30", "-20", "-10", "0", "10", "20", "30"]
			end_z = ["-50","-40","-30","-20","-10","0","10","20","30","40","50","60"]#,"-40","-30","-20","-10","0","10","20","30","40","50"] #-30", "-20", "-10", "0", "10", "20", "30", "40"]
		...
		if ( "ddG-of-pH-insertion" in test_name ):
			...
			start_z = ["-60","-50","-20","-10","0","-40","-30","10","20","30","40","50"]#,"-50","-40","-30","-20","-10","0","10","20","30","40"] #["-40", "-30", "-20", "-10", "0", "10", "20", "30"]
			end_z = ["-50","-40","-10","0","10","-30","-20","20","30","40","50","60"]#,"-40","-30","-20","-10","0","10","20","30","40","50"] #-30", "-20", "-10", "0", "10", "20", "30", "40"]
        
		#hpc_util.submit_array_rockfish_job(outdir,case, pH)
		
		for j in range(len(start_z)):
 
			#the default lipid composition is DLPC, based on the test, the lipid composition and temperature at 30. 
			...
			if(interface=="1"):
				...
				if( ( case=="1a11" ) or ( case=="2nr1" ) ):
					...
					arguments = " -overwrite -in:file:s " +  pdbfile + " -mp:setup:spanfiles " + spanfile + " -parser:script_vars sfxn_weights=" + energy_fxn + " -parser:script_vars interface=" + interface + " -parser::script_vars start_z=" + start_z[j] + " -parser::script_vars end_z=" + end_z[j] + " -parser::script_vars flag_axis=" + flag_axis[0] + "  -parser:protocol " + xml_script + " -mp:lipids:composition DMPC -mp:lipids:temperature 30.0"
				else:
					arguments = " -overwrite -in:file:s " +  pdbfile + " -mp:setup:spanfiles " + spanfile + " -parser:script_vars sfxn_weights=" + energy_fxn + " -parser:script_vars interface=" + interface + " -parser::script_vars start_z=" + start_z[j] + " -parser::script_vars end_z=" + end_z[j] + " -parser::script_vars flag_axis=" + flag_axis[0] + "  -parser:protocol " + xml_script + " -mp:lipids:composition DLPC -mp:lipids:temperature 30.0"
